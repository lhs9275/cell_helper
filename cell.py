...
change_colums = 0
df_data = pd.read_excel('reservation.xlsx', index_col=0)
...
with pd.ExcelWriter('tennis_court_schedule.xlsx', engine='openpyxl') as writer:
    ...
    df_sch.to_excel(writer, sheet_name='Sheet1', startcol=0, startrow=0, header=True, index=True)
    ...
    for col_num, value in enumerate(new_column_names):
        worksheet.column_dimensions[worksheet.cell(row=1, column=col_num+1).column_letter].width = 13

    # 높이 설정
    for row_num, value in enumerate(new_index_values):
        worksheet.row_dimensions[row_num + 2].height = 25


    worksheet['A1'] = today_date
